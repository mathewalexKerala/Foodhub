...
module.exports = {
  ...
  account_control: async (req, res) => {
    ...
    if (foundValues) {
      users
        .updateOne({ _id: req.params.id }, { $set: toUpdate })
        .then(() => {
    ...
    if (req.body.action) {
      if (req.body.action == "delete") {
        ...
        users
          .updateOne({ _id: req.params.id }, { $set: { status: "banned" } })
          .then(() => {
            apiRes.message = "Account banned Successfully!";
            apiRes.success = true;
          })
          .catch((err) => {
            apiRes.message = "Error detected while updating!";
            console.log(err);
          })
          .then(() => {
            res.status(200).json(apiRes);
        ...
        users
          .updateOne({ _id: req.params.id }, { $set: { status: "active" } })
          .then(() => {
  ...
  category_create: async (req, res) => {
    ...
    if (req.body.name && req.body.tags && req.body.action) {
      ///&& req.body.action
      ...
      if (req.file) {
        main
          .uploadimage(req, "category", "cat-", true)
          .then((data) => {
            // true = crop
  ...
  category_delete: async (req, res) => {
    ...
    try {
      isProductExistUnderCat = await products.countDocuments({
    ...
    if (isProductExistUnderCat == 0) {
      categories.findOne({ _id: req.params.cid }).then((data) => {
        if (data && data.image) {
          main.deleteFile(
            path.join(__dirname, "../public/uploads/category/") + data.image
          );
        }
      });
      categories
      ...
      apiRes.message =
        "There is one or more product(s) under this category, please update the product(s) first!";
      res.status(apiRes.status).json(apiRes);
    }
  },
  product_action: async (req, res) => {
        ...
        if (req.body.id) {
          ...
          if (dataToUpload2.image) {
            return products.updateOne(
              ...
              {
                $set: dataToUpload,
                $push: { image: { $each: dataToUpload2.image } },
              }
            );
          } else {
            return products.updateOne(
  ...
  product_delete: (req, res) => {
    ...
    products
      .updateOne({ _id: req.params.cid }, { $set: { "state.deleted": true } })
      .then((data) => {
        console.log(data);
  ...
  banner_update: (req, res, next) => {
        ...
        if (req.body.id) {
          if (req.file) {
            banners.findOne({ _id: req.body.id }).then((data) => {
              if (data.image) {
                main.deleteFile(
                  path.join(__dirname, "../public/uploads/banner/") + data.image
                );
              }
            });
          }
    ...
    if (
      ...
      if (req.file) {
        main
          .uploadimage(req, "banner", "bnr-", false)
          .then((data) => {
            // false = crop
            apiRes.message = data.message;
            apiRes.success = true;
  ...
  admin_profile_edit: (req, res) => {
    ...
    if (readyToUpdate) {
      admins
        .updateOne({ _id: req.session.adminid }, { $set: dataToUpdate })
        .then(() => {
  ...
  apiResponse: function (req, res, next) {
